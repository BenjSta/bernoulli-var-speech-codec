...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, padding_left, center=False, return_stft=False):
    # if torch.min(y) < -1.:
    ...
    if padding_left == -1:
        assert (n_fft-hop_size)/2 == int((n_fft-hop_size)/2), 'No symmetric padding possible'
        pl = int((n_fft-hop_size)/2)
        pr = pl
    else:
        pl = padding_left
        pr = win_size - padding_left - hop_size

    y = torch.nn.functional.pad(y.unsqueeze(1), (pl, pr), mode='reflect')
    y = y.squeeze(1)
