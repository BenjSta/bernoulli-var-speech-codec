...
for testset in ['vctk', 'tsp']:
    result_dir = 'results_' + testset
    if testset == 'vctk':
        clean_train, clean_val, clean_test = paths["clean"]
    else:
        clean_test = sorted(glob.glob('/media/DATAslow/shared/stahl/mtedx_iwslt2021/48k/**/*.wav'))
    ...
    np.random.seed(1)
    test_test_examples = np.random.choice(len(clean_test), 10, replace=False)
    np.random.seed()
    ...
    def opus(y, fs, bitrate=6, fs_opus=48000):
        with tempfile.NamedTemporaryFile(suffix='.wav', mode='r+') as tmpfile1, tempfile.NamedTemporaryFile(suffix='.wav', mode='r+') as tmpfile3, tempfile.NamedTemporaryFile(suffix='.opus', mode='r+') as tmpfile2:
            if fs != fs_opus:
                y = scipy.signal.resample_poly(y, fs_opus, fs)
            soundfile.write('%s' % tmpfile1.name, y, fs_opus)
            ...
            subprocess.call('/home/stahl/miniconda3/envs/spear/bin/ffmpeg -y -i %s -ar %d %s' %
                            (tmpfile2.name, fs_opus, tmpfile3.name), stdout=subprocess.DEVNULL,
                            stderr=subprocess.STDOUT, shell=True)
            y, _ = soundfile.read('%s' % tmpfile3.name)
            if fs != fs_opus:
                y = scipy.signal.resample_poly(y, fs, fs_opus)
    ...
    def audiodec_fn(x, fs):
        with tempfile.NamedTemporaryFile(suffix='.wav', mode='r+') as tmpfile:
            if fs != 24000:
                x = scipy.signal.resample_poly(x, 24000, fs)
            
            y = torch.from_numpy(x.astype('float32')).to(device)
            l = y.shape[0]
            y = y[None, None, :]
            y = audiodec_model.tx_encoder.encode(y)
            y = audiodec_model.tx_encoder.quantize(y)
            y = audiodec_model.rx_encoder.lookup(y)
            y = audiodec_model.decoder.decode(y)[0, 0, :l].detach().cpu().numpy() # correct for delay of 192 samples
    ...
    encode_fn = audiodec_model.tx_encoder.encode
    forward_fn = audiodec_model.tx_encoder.forward
    audiodec_model.tx_encoder.forward = encode_fn
    macs, params = get_model_complexity_info(
        audiodec_model.tx_encoder,
        (1, 240000,),
        as_strings=False,
        print_per_layer_stat=False,
        verbose=False,
    )
    audiodec_model.tx_encoder.forward = forward_fn
    print('Audiodec Enc', macs/1e10)

    decode_fn = audiodec_model.decoder.decode
    forward_fn = audiodec_model.decoder.forward
    audiodec_model.decoder.forward = decode_fn
    macs, params = get_model_complexity_info(
        audiodec_model.decoder,
        (800, 64),
        as_strings=False,
        print_per_layer_stat=False,
        verbose=False,
    )
    audiodec_model.decoder.forward = forward_fn
    print('Audiodec Dec', macs/1e10)
    ...
    sws = ['clean',
        ...
        'lyra9_2', 'encodec1_5', 'encodec3', 'encodec6', 'encodec12', 'audiodec8',
        'fixed16', 'fixed24',
        'fixed32', 'fixed64',
    ...
    sigs_processed = [clean_all, vocoded_all, vocoded_big_all, vocoded_bigsym_all,
            opus6_16_all, opus8_16_all, opus10_16_all, opus12_16_all,
            opus6_all, opus8_all, opus10_all, opus12_all, lyra3_2_all, lyra6_all,
            lyra9_2_all, encodec1_5_all, encodec3_all, encodec6_all, encodec12_all, audiodec8_all,
    ...
    for idx, (y,) in enumerate(tqdm.tqdm(test_dataloader)):
        if idx in test_test_examples or True:
            with torch.no_grad():
                ...
                # opus6_16_all.append(10**(-MAKEUP_GAIN/20)*opus(10**(MAKEUP_GAIN/20)* y[0, :].numpy(), 48000, 6, 16000))
                # opus8_16_all.append(10**(-MAKEUP_GAIN/20)*opus(10**(MAKEUP_GAIN/20)* y[0, :].numpy(), 48000, 8, 16000))
                # opus10_16_all.append(10**(-MAKEUP_GAIN/20)*opus(10**(MAKEUP_GAIN/20)* y[0, :].numpy(), 48000, 10, 16000))
                # opus12_16_all.append(10**(-MAKEUP_GAIN/20)*opus(10**(MAKEUP_GAIN/20)* y[0, :].numpy(), 48000, 12, 16000))
                ...
                audiodec8_all.append(10**(-MAKEUP_GAIN/20)*audiodec_fn(10**(MAKEUP_GAIN/20)*y[0, :].numpy(), 48000)) #
    ...
    for sw, sigs_method in zip(sws, sigs_processed):
        if sw == 'audiodec8':
            ...
            f0_rmse, voiced_acc = compute_f0_rmse(clean_all, sigs_method, 48000)

            df.loc[:, 'pesq'] = pesq
            df.loc[:, 'nisqa_mos16'] = nisqa16_mos
            df.loc[:, 'nisqa_mos48'] = nisqa48_mos
            df.loc[:, 'visqol'] = visqol
            df.loc[:, 'f0_rmse'] = f0_rmse
            df.loc[:, 'voiced_acc'] = voiced_acc
            # df.loc[:,'wacc'] = wacc
            df.loc[:, 'lengths'] = lengths_all
            fileName = os.path.join(result_dir, sw + '.csv')
            for i in range(len(sigs_method)):
                wavname = os.path.join(wavPath, sw + '_%d.flac' % i)
                soundfile.write(wavname, sigs_method[i], 48000)
    ...
    #     df.loc[:,name] = data
    #     df.loc[:,'lengths'] = lengths_all
    #     df.to_csv(os.path.join(result_dir, 'Distances_64.csv'))